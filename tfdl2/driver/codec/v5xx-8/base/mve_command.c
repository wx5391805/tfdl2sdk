                                                        ...
                                                        struct file *filep)
{
    ...
    if (false == is_role_enabled(role))
    {
        ...
        MVE_LOG_PRINT(&mve_rsrc_log_session8,
                      MVE_LOG_ERROR,
    ...
    {
        session = mve_session_create8(filep);
        if (NULL == session)
        {
        ...
        {
            mve_session_set_role8(session, role);
        }
    }
...
struct mve_response *mve_command_execute8(struct mve_base_command_header *header,
                                         void *data,
                                         struct file *filep)
{
#define MVE_LOG_COMMAND(_severity)                                                                                   \
    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session8,                                                                     \
                          _severity,                                                                                 \
                          session,                                                                                   \
                          "mve_command_execute8(header={cmd=%u \"%s\", size=%u, data[0]=%u}, data=0x%x, file=0x%x).", \
                          header->cmd,                                                                               \
    ...
    if (NULL == ret)
    {
        MVE_LOG_PRINT(&mve_rsrc_log8, MVE_LOG_ERROR, "Failed to allocate memory for the ioctl return struct.");
        return NULL;
    }
...
//    printk("%s : cmd %d %s enter\n", __func__, header->cmd, mve_command_to_string(header->cmd));
    ...
    switch (header->cmd)
    {
        ...
        case MVE_BASE_ACTIVATE_SESSION:
        {
            ...
            if (NULL != version)
            {
                res = mve_session_activate8(session, version, fw_secure_desc);
                if (false != res)
                {
                    ret->data = version;
        ...
        case MVE_BASE_ENQUEUE_FLUSH_BUFFERS:
        {
            ...
            ret->error = mve_session_enqueue_flush_buffers8(session, flush);
            break;
        }
        case MVE_BASE_ENQUEUE_STATE_CHANGE:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_session_enqueue_state_change8(session, state);
            break;
        }
        case MVE_BASE_GET_EVENT:
        {
            ...
            event = mve_session_get_event8(session, timeout);
            if (NULL == event)
            {
        ...
        {
            ret->error = mve_session_set_paramconfig8(session,
                                                     header->size,
                                                     data,
                                                     &ret->firmware_error,
                                                     header->cmd);
        ...
        case MVE_BASE_REGISTER_BUFFER:
        {
            ...
            {
                ret->error = mve_session_buffer_register8(session, port_index, descriptor);
            }
            break;
        }
        case MVE_BASE_UNREGISTER_BUFFER:
        {
            ...
            {
                MVE_LOG_COMMAND(MVE_LOG_INFO);
                ret->error = mve_session_buffer_unregister8(session, buffer_id);
            }
            break;
        }
        case MVE_BASE_FILL_THIS_BUFFER:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_session_buffer_enqueue8(session, param, false);
            break;
        }
        case MVE_BASE_EMPTY_THIS_BUFFER:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_session_buffer_enqueue8(session, param, true);
            break;
        }
        case MVE_BASE_NOTIFY_REF_FRAME_RELEASE:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_session_buffer_notify_ref_frame_release8(session, buffer_id);
            break;
        }
        case MVE_BASE_REQUEST_MAX_FREQUENCY:
        {
            ...
            mver_dvfs_request_max_frequency8();
            ret->error = MVE_BASE_ERROR_NONE;
            break;
        }
        case MVE_BASE_READ_HW_INFO:
        {
            ...
            else
            {
                ...
                hw_info->fuse = mver_reg_get_fuse8();
                hw_info->version = mver_reg_get_version8();
                hw_info->ncores = mver_scheduler_get_ncores8();
            }
            break;
        }
        case MVE_BASE_RPC_MEM_ALLOC:
        {
            ...
            fd = *((int32_t *)data);
            ret->error = mve_session_handle_rpc_mem_alloc8(session, fd);
#endif      /* EMULATOR */
            break;
        }
        case MVE_BASE_RPC_MEM_RESIZE:
        {
            ...
            fd = *((int32_t *)data);
            ret->error = mve_session_handle_rpc_mem_resize8(session, fd);
#endif      /* EMULATOR */
            break;
        }
#ifdef UNIT
        case MVE_BASE_DEBUG_READ_REGISTER:
            ...
            else
            {
                ...
                value = mver_reg_read328(base + offset);
        ...
        case MVE_BASE_DEBUG_WRITE_REGISTER:
            ...
            WARN_ON(header->size != sizeof(uint32_t) * 2);
            {
                ...
                mver_reg_write328(base + offset, value);
                mver_reg_put_coresched_bank8(&regs);
            }
            break;
        case MVE_BASE_DEBUG_INTERRUPT_COUNT:
            ...
            {
                *((uint32_t *)ret->data) = mve_rsrc_data8.interrupts;
                ret->size = sizeof(uint32_t);
            }
            break;
        case MVE_BASE_DEBUG_SEND_COMMAND:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_session_enqueue_message8(session, ptr[0], ptr[1], &ptr[2]);
            break;
        }
        case MVE_BASE_DEBUG_FIRMWARE_HUNG_SIMULATION:
        {
            ...
            mve_session_firmware_hung_simulation8(session, on);
#endif

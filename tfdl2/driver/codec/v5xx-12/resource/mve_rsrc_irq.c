 ...
 */
void mver_irq_handler_bottom12(struct work_struct *work)
{
    ...
    for (lsid = 0; lsid < mve_rsrc_data12.nlsid; lsid++)
    {
        ...
        if (irq_set != 0)
        {
            ...
            mdelay(irq_delay);
#endif
            mver_scheduler_handle_irq12(lsid);
        }
    }
}
 ...
 */
irqreturn_t mver_irq_handler_top12(int irq, void *dev_id)
{
    uint32_t irq_vector;
    tCS *regs;
    unsigned lsid;
    ...
    regs = mver_reg_get_coresched_bank_irq12();
    irq_vector = mver_reg_read3212(&regs->IRQVE);
...
void mver_irq_handler_init12(struct device *dev)
{
#if defined(CONFIG_SYSFS) && defined(_DEBUG)
    int err;
#endif
    bottom_half.irq = 0;
    INIT_WORK(&bottom_half.work, mver_irq_handler_bottom12);

    mver_work_queue = (struct workqueue_struct *)create_workqueue("mv500-12--wq");
...
#ifdef UNIT
    mve_rsrc_data12.interrupts = 0;
#endif
}

void mver_irq_handler_deinit12(struct device *dev)
{
...
void mver_irq_signal_mve12(mver_session_id session)
{
    if (false != mve_rsrc_data12.hw_interaction)
    {
        tCS *regs = mver_reg_get_coresched_bank12();
        enum LSID lsid;

        lsid = mver_scheduler_get_session_lsid12(session);

        if (lsid != NO_LSID)
        {
            mver_reg_write3212(&regs->LSID[lsid].IRQHOST, 1);
        }
...
int mver_irq_enable12(void)
{
    if (-1 != mve_rsrc_data12.irq_nr && 0 == mve_rsrc_data12.irq_enable_count++)
    {
        int irq_trigger = mve_rsrc_data12.irq_flags & IRQF_TRIGGER_MASK;
        int ret = request_irq(mve_rsrc_data12.irq_nr, mver_irq_handler_top12,
                              IRQF_SHARED | irq_trigger,
                              MVE_RSRC_DRIVER_NAME, &mve_rsrc_data12);
        if (0 != ret)
        {
            mve_rsrc_data12.irq_enable_count--;
            return -ENXIO;
        }
    }
...
void mver_irq_disable12(void)
{
    if (-1 != mve_rsrc_data12.irq_nr && 1 == mve_rsrc_data12.irq_enable_count--)
    {
        free_irq(mve_rsrc_data12.irq_nr, &mve_rsrc_data12);
        cancel_work_sync(&bottom_half.work);
    }
}

EXPORT_SYMBOL(mver_irq_signal_mve12);
EXPORT_SYMBOL(mver_irq_enable12);
EXPORT_SYMBOL(mver_irq_disable12);

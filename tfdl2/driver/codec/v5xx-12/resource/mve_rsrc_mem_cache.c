...
void mve_rsrc_mem_cache_init12(void)
{
...
void mve_rsrc_mem_cache_deinit12(void)
{
...
static struct kmem_cache *mve_rsrc_get_cachep(uint32_t size)
{
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log12, MVE_LOG_ERROR, "No allocation slab found. size=%u.", size);
    }

    return cachep;
}

void *mve_rsrc_mem_cache_alloc12(uint32_t size, uint32_t flags)
{
    void *ptr;
...
void mve_rsrc_mem_cache_free12(void *ptr, uint32_t size)
{
    struct kmem_cache *cachep;
...
EXPORT_SYMBOL(mve_rsrc_mem_cache_alloc12);
EXPORT_SYMBOL(mve_rsrc_mem_cache_free12);

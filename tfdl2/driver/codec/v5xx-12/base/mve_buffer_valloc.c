...
struct mve_buffer_external *mve_buffer_valloc_create_buffer_external12(struct mve_buffer_info *info)
{
...
void mve_buffer_valloc_destroy_buffer_external12(struct mve_buffer_external *buffer)
{
    MVE_RSRC_MEM_CACHE_FREE(buffer, sizeof(struct mve_buffer_external));
}

bool mve_buffer_valloc_map_physical_pages12(struct mve_buffer_external *buffer)
{
    uint32_t size;
    void *ptr;
    uint32_t num_pages;
    ...
    pages = mve_rsrc_mem_map_virt_to_phys12(ptr - offset, size, buffer->mapping.write);
    if (NULL == pages)
    {
        return false;
    }
...
bool mve_buffer_valloc_unmap_physical_pages12(struct mve_buffer_external *buffer)
{
    /* mve_mem_unmap_virt_to_phys frees the memory allocated for the array
     * buffer->pages. */
    mve_rsrc_mem_unmap_virt_to_phys12(buffer->mapping.pages, buffer->mapping.num_pages);
...
void mve_buffer_valloc_set_owner12(struct mve_buffer_external *buffer,
                                 ...
                                 enum mve_port_index port)
{
    ...
    for (i = 0; i < buffer->mapping.num_pages; ++i)
    {
        ...
        sync_func(&mve_rsrc_data12.pdev->dev, buffer->mapping.pages[i] + offset, size, direction);
    }
}

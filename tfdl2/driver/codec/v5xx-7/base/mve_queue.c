...
struct mve_queue *mve_queue_create7(uint32_t size)
{
...
void mve_queue_destroy7(struct mve_queue *queue)
{
    if (NULL != queue)
    {
        MVE_RSRC_MEM_VFREE(queue->buffer);
...
void mve_queue_interrupt7(struct mve_queue *queue)
{
    queue->interrupted = true;
    wake_up_interruptible(&queue->wait_queue);
}

bool mve_queue_interrupted7(struct mve_queue *queue)
{
    return queue->interrupted;
}

struct mve_base_event_header *mve_queue_wait_for_event7(struct mve_queue *queue, uint32_t timeout)
{
    ...
    event = mve_queue_create_event7(header.code, header.size);
    if (NULL == event)
    {
...
void mve_queue_add_event7(struct mve_queue *queue,
...
struct mve_base_event_header *mve_queue_create_event7(int code, int size)
{
    struct mve_base_event_header *event;
...
bool mve_queue_event_ready7(struct mve_queue *queue)
{

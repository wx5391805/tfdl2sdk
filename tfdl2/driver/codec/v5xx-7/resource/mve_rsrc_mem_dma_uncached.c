...
struct mve_rsrc_dma_mem_t *mve_rsrc_dma_mem_alloc_uncached7(uint32_t size)
{
    ...
    if (NULL != mem)
    {
        ...
        mem->cpu_addr = dma_alloc_coherent(&mve_rsrc_data7.pdev->dev, size, &mem->dma_handle, GFP_KERNEL);
...
void mve_rsrc_dma_mem_free_uncached7(struct mve_rsrc_dma_mem_t *mem)
{
    struct dma_mem_uncached_t *ptr = (struct dma_mem_uncached_t *)mem;

    dma_free_coherent(&mve_rsrc_data7.pdev->dev, ptr->header.size, ptr->cpu_addr, ptr->dma_handle);
    if (NULL != ptr->pages)
    {
...
void mve_rsrc_dma_mem_clean_cache_uncached7(struct mve_rsrc_dma_mem_t *mem)
{
    /* Nop */
    mve_rsrc_mem_flush_write_buffer7();
}

void mve_rsrc_dma_mem_invalidate_cache_uncached7(struct mve_rsrc_dma_mem_t *mem)
{
    /* Nop */
    mve_rsrc_mem_flush_write_buffer7();
}

void *mve_rsrc_dma_mem_map_uncached7(struct mve_rsrc_dma_mem_t *mem)
{
    struct dma_mem_uncached_t *ptr = (struct dma_mem_uncached_t *)mem;
...
void mve_rsrc_dma_mem_unmap_unchached7(struct mve_rsrc_dma_mem_t *mem)
{
    /* Nop */
}

phys_addr_t *mve_rsrc_dma_mem_get_pages_uncached7(struct mve_rsrc_dma_mem_t *mem)
{
    struct dma_mem_uncached_t *ptr = (struct dma_mem_uncached_t *)mem;

...
void mve_rsrc_mem_cache_init7(void)
{
...
void mve_rsrc_mem_cache_deinit7(void)
{
...
static struct kmem_cache *mve_rsrc_get_cachep(uint32_t size)
{
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log7, MVE_LOG_ERROR, "No allocation slab found. size=%u.", size);
    }

    return cachep;
}

void *mve_rsrc_mem_cache_alloc7(uint32_t size, uint32_t flags)
{
    void *ptr;
...
void mve_rsrc_mem_cache_free7(void *ptr, uint32_t size)
{
    struct kmem_cache *cachep;
...
EXPORT_SYMBOL(mve_rsrc_mem_cache_alloc7);
EXPORT_SYMBOL(mve_rsrc_mem_cache_free7);
